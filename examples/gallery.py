...
and the HTML table in both its layouts (flat and stacked), so they can be eyeballed
together (visual-regression review, prepping doc assets). Both rendering paths are
exercised: the base loader
...
def _host(factory: "Callable[[], KnockoutStage]") -> Builder:
    def build() -> "tuple[str, str, str]":
        ...
        return (
            diagram.render("svg"),
            diagram.render("html", layout="flat"),
            diagram.render("html", layout="stacked"),
        )
...
<header>
  ...
  (flat and stacked). Light/dark follows your browser's color-scheme setting — flip it
  to see both themes.</p>
...
def build_page() -> str:
    ...
    for filename, note, builder in SPECS:
        ...
        parts.append(
            f'<div class="pane"><h3>HTML table — flat</h3>{_frame(html_flat)}</div>'
        )
        parts.append(
            f'<div class="pane"><h3>HTML table — stacked</h3>{_frame(html_stacked)}</div>'
        )
        parts.append("</div>")
